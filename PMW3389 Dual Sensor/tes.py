...
def extract_bytes(text: str):
    """Extract all hex bytes (0x00..0xFF) from a firmware array string"""
    return re.findall(r"0x[0-9a-fA-F]{2}", text)

t1 = input("""const unsigned char firmware_data[] PROGMEM = {
...
t2 = input("""const unsigned char firmware_data[] = {
0x01, 0xe8, 0xba, 0x26, 0x0b, 0xb2, 0xbe, 0xfe, 0x7e, 0x5f, 0x3c, 0xdb, 0x15, 0xa8, 0xb3, 
0xe4, 0x2b, 0xb5, 0xe8, 0x53, 0x07, 0x6d, 0x3b, 0xd1, 0x20, 0xc2, 0x06, 0x6f, 0x3d, 0xd9, 
0x11, 0xa0, 0xc2, 0xe7, 0x2d, 0xb9, 0xd1, 0x20, 0xa3, 0xa5, 0xc8, 0xf3, 0x64, 0x4a, 0xf7, 
0x4d, 0x18, 0x93, 0xa4, 0xca, 0xf7, 0x6c, 0x5a, 0x36, 0xee, 0x5e, 0x3e, 0xfe, 0x7e, 0x7e, 
...
bytes1 = extract_bytes(t1)
bytes2 = extract_bytes(t2)

if bytes1 == bytes2:
    print("✅ The two firmware arrays are identical.")
else:
    print("❌ They differ.")
    # Check where the difference starts
    min_len = min(len(bytes1), len(bytes2))
    for i in range(min_len):
        if bytes1[i] != bytes2[i]:
            print(f"First difference at index {i}: {bytes1[i]} != {bytes2[i]}")
            break
    if len(bytes1) != len(bytes2):
        print(f"Lengths differ: {len(bytes1)} vs {len(bytes2)}")
